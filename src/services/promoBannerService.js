...
const PUBLIC_API_URL = `${API_BASE_URL}/api/promo-banner`;
const ADMIN_API_URL = `${API_BASE_URL}/api/admin/promo-banner`;
...
// Public API - Get promo banner data
export const getPromoBannerData = async () => {
  const response = await fetch(PUBLIC_API_URL);
  if (!response.ok) {
    throw new Error('Failed to fetch promo banner data');
  }
